...
export const handlers = [
  ...
  http.get(`${API_URL}/tweets/:id/summary`, ({ params, request }) => {
    const { id } = params as { id: string };
    const tweet = getTweet(id);
    const urlSearchParams = new URL(request.url).searchParams;
    const locale = urlSearchParams.get('locale') || 'en-US';
    console.log(`Locale for summary: ${locale}`);
    if (!tweet) {
      return HttpResponse.json({ message: `Tweet "${id}" not found` }, { status: 404 });
    }
    let summary =
      'This is a mock summary of the tweet content. In a real scenario, this would be generated by an AI model.';
    if (locale === 'ar-EG') {
      summary =
        'هذا ملخص تجريبي لمحتوى التغريدة. في سيناريو حقيقي، سيتم إنشاؤه بواسطة نموذج ذكاء اصطناعي.';
    }
    const data = {
      id: id,
      summary: summary,
    ...
    return HttpResponse.json(
      { success: true, message: 'Retweet undone successfully.', data: data },
      { status: 200 },
    );
  }),
