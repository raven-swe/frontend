...
const settingsService = vi.hoisted(() => ({
  getMutedPaginated: vi.fn().mockResolvedValue({
    data: [],
    cursor: null,
  }),
}));
...
const createWrapper = async () => {
  return await mountSuspended(MutedPage, {
    global: {
      mocks: {
        $router: routerMock,
      },
...
describe('pages/settings/muted.vue', () => {
  it('render header and description', async () => {
    const wrapper = await createWrapper();
    expect(wrapper.text()).toContain('Muted accounts');
  ...
  it('passes correct fetcher function to UserList', async () => {
    ...
    expect(settingsService.getMutedPaginated).toHaveBeenCalledWith({
      cursor: mockCursor,
      signal: mockSignal,
    });
  });
