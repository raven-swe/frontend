...
import tweetLikesGetHandler from '~~/server/api/tweets/[id]/likes.get';
...
describe('GET /api/tweets/[id]/likes', () => {
  ...
  it('returns tweet likers for valid tweet id', async () => {
    const mockResponse = {
      success: true,
      data: [
        {
    ...
    const response = await tweetLikesGetHandler(event);

    expect(mockServerApiFetch).toHaveBeenCalledWith('/tweets/tw-id/likes', {
      method: 'GET',
  ...
  it('should handle missing tweet id parameter', async () => {
    ...
    await expect(tweetLikesGetHandler(event)).rejects.toEqual(
      createError({
        statusCode: 422,
