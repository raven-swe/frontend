...
describe('POST /api/auth/password/reset', () => {
  it('returns access and refresh tokens on success', async () => {
    ...
    const handler = (await import('../../../../../server/api/auth/password/reset.post')).default;
    const res = await handler(
      makeEvent({ confirmationToken: 'token123', newPassword: 'NewPass123!' }),
    );

    expect(fetchMock).toHaveBeenCalledWith('https://api.example.com/auth/password/reset', {
      method: 'POST',
  ...
  it('throws validation error (422) when token or password is missing', async () => {
  ...
  it('throws validation error (422) when password is too short', async () => {
    const { FetchError } = await import('ofetch');
    const validationError = {
      error: {
        code: 'VALIDATION_ERROR',
    ...
    // @ts-expect-error - Mock FetchError accepts 3 parameters
    fetchMock.mockRejectedValueOnce(new FetchError('Password too short', validationError, 422));

    const handler = (await import('../../../../../server/api/auth/password/reset.post')).default;
    await expect(
      handler(makeEvent({ confirmationToken: 'token123', newPassword: 'short' })),
  ...
  it('throws error (400) when token is invalid or expired', async () => {
    const { FetchError } = await import('ofetch');
    ...
    const handler = (await import('../../../../../server/api/auth/password/reset.post')).default;
    await expect(
      handler(makeEvent({ confirmationToken: 'invalid_token', newPassword: 'NewPass123!' })),
    ).rejects.toMatchObject({
      statusCode: 400,
  ...
  it('throws error (400) when user not found', async () => {
    ...
    // @ts-expect-error - Mock FetchError accepts 3 parameters
    fetchMock.mockRejectedValueOnce(new FetchError('User not found', userNotFoundError, 400));

    const handler = (await import('../../../../../server/api/auth/password/reset.post')).default;
    await expect(
      handler(makeEvent({ confirmationToken: 'token123', newPassword: 'NewPass123!' })),
    ).rejects.toMatchObject({
      statusCode: 400,
